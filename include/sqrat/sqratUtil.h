...
inline string FormatTypeError(HSQUIRRELVM vm, SQInteger idx, const SQChar *expectedType) {
    auto fmtstr = _SC("wrong type (%s expected)");
    const auto l = SQRAT_SPRINTF(nullptr, 0, fmtstr, expectedType);
    string err(l + 1, '\0');
    SQRAT_SPRINTF(&err[0], err.size(), fmtstr, expectedType);
...
#pragma warning(disable: 4100)
  ...
  decltype(auto) apply_helper(Func pf, SQRAT_STD::index_sequence<Indexes...>, Tuple &args)
  {
  ...
  decltype(auto) apply_member_helper(C *ptr, Member pf, SQRAT_STD::index_sequence<Indexes...>,
                           Tuple &args)
  {
